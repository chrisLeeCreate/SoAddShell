...
//		__android_log_print(ANDROID_LOG_INFO, "JNITag", "phdr =  0x%p\n", phdr);
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNITag", "dyn_vadd =  0x%x, dyn_size =  0x%x", dyn_vaddr,
                        dyn_size);
    flag = 0;
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNITag", "st_value = %d, st_size = %d", info->st_value,
                        info->st_size);
    return 0;
    _error:
    return -1;
...
void init_getString() {
    ...
    if (mprotect((void *) ((base + info.st_value) / PAGE_SIZE * PAGE_SIZE), npage,
...
Java_com_example_shelldemo2_MainActivity_getString(JNIEnv *env, jobject thiz) {
#if defined(__arm__)
...
#define ABI "unknown"
#endif
    std::string hello = "Hello from lishaowei";
